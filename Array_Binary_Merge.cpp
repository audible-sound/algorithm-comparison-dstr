...
struct Job {
    ...
    Job() : role(""), skills(nullptr), skillCount(0), skillCapacity(0) {}
...
struct Candidate {
    ...
    Candidate() : role("Candidate"), skills(nullptr), skillCount(0), skillCapacity(0) {}
...
public:
    ...
    void addJob(const Job& j) { ensureCapacity(size + 1); arr[size++] = j; }
    ...
    static Job parseLine(const string& line) {
        ...
        trim(job.role);
        if (!job.role.empty() && job.role.front() == '"') job.role.erase(0, 1);
        if (!job.role.empty() && job.role.back() == '"') job.role.pop_back();

        // Extract skills
        size_t posExp = desc.find("experience in ");
        string skillsPart;
        if (posExp != string::npos) skillsPart = desc.substr(posExp + 13);
...
public:
    ...
    static Candidate parseLine(const string& line) {
        ...
        while (getline(ss, token, ',')) {
            trim(token);
            if (!token.empty() && isValidSkill(token)) cand.addSkill(token);
        }
        return cand;
    }
...
struct Result { Job* job; int score; };
...
void merge(Result arr[], int l, int m, int r) {
    ...
    int i = 0, j = 0, k = l;
    while (i < n1 && j < n2) arr[k++] = (L[i].score >= R[j].score) ? L[i++] : R[j++];
...
int main() {
    while(true){
        ...
        if (userType == "1") {
            // Load jobs
            JobArray jobs;
            if (!jobs.loadCSV("job_description.csv")) return 1;
            cout << "\n-------------- Job Seeker ---------------\n";
            ...
            while (getline(ss, token, ',')) {
                ...
                if (!token.empty()) {
                    ...
                    if (!exists) userSkills[userSkillCount++] = token;
                }
            }
            ...
            for (int i = 0; i < n; i++) {
                ...
                for (int u = 0; u < userSkillCount; u++)
                    if (binarySearchSkill(j.skills, j.skillCount, userSkills[u]) != -1) matchCount++;
            ...
            for (int i = 0; i < jobs.getSize(); i++) {
                ...
                for (int k = 0; k < j.skillCount; k++) totalMemory += j.skills[k].capacity();
            }
            ...
            while (getline(ss, token, ',')) {
                ...
                if (!token.empty()) {
                    ...
                    for (int i = 0; i < jobSkillCount; i++) {
                        if (equalsIgnoreCase(jobSkills[i], token)) { exists = true; break; }
                    }
                    if (!exists) jobSkills[jobSkillCount++] = token;
                }
            }
            ...
            for (int i = 0; i < n; i++) {
                ...
                for (int j = 0; j < jobSkillCount; j++)
                    if (binarySearchSkill(c.skills, c.skillCount, jobSkills[j]) != -1) matchCount++;
            ...
            for (int i = 0; i < candidates.getSize(); i++) {
                ...
                for (int k = 0; k < c.skillCount; k++) totalMemory += c.skills[k].capacity();
            }
